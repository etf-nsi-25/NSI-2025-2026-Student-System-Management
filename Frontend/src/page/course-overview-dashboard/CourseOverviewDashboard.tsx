...
const CourseOverviewDashboard = () => {
  ...
  const [toast] = useState<ReactElement<CToastProps> | undefined>(undefined);
  ...
  const colors = { success: '#68B99A', danger: '#dc3545' };

  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://localhost:5283/api/faculty/courses/${courseId}/overview`, {
        ...
        console.error("Backend unreachable, showing placeholders", error);
      }
    };
  ...
  const attendanceColor = isAttendanceLow ? colors.danger : '#2eb85c';
  ...
  return (
    <div className="p-5" style={{ backgroundColor: pageBackground, minHeight: '100vh' }}>
      ...
      <div className="bg-white p-4 rounded-4 shadow-sm mb-4 border-0">
        ...
        {courseData.exams.length > 0 ? courseData.exams.map((exam) => (
          <div key={exam.id} className="d-flex align-items-center mb-3 p-3 rounded-3 shadow-sm" style={{ backgroundColor: '#f0f7fa' }}>
            <div className="rounded-3 d-flex align-items-center justify-content-center text-white fw-bold me-3" 
                 style={{ width: '55px', height: '45px', backgroundColor: exam.status === 'FAILED' ? colors.danger : brandBlue, fontSize: '1.2rem' }}>
              {exam.status === 'FAILED' ? '!' : '✓'}
            </div>
        ...
        )) : <p className="text-muted italic px-3">No exams available yet.</p>}
      </div>
      ...
      <div className="bg-white p-4 rounded-4 shadow-sm border-0">
        ...
        {courseData.assignments.length > 0 ? courseData.assignments.map((task) => (
        ...
        )) : <p className="text-muted italic px-3">No assignments available yet.</p>}
      </div>
