...
public class UserServiceTests
{
    ...
    public async Task CreateUserAsync_ShouldReturnUserId_OnSuccessfulCreation()
    {
        ...
        _mockHasher.Setup(h => h.HashPassword(password)).Returns(new byte[] { 1, 2, 3 });
    ...
    public async Task UpdateUserAsync_ShouldReturnTrueAndPublishEvent_WhenRoleChanges()
    {
        ...
        var existingUser = User.Create("u1", new byte[1], "F", "L", Guid.NewGuid(), oldRole, "12345");
...
{
    var userId = Guid.NewGuid();
    var activeUser = User.Create("u", new byte[1], "F", "L", Guid.NewGuid(), UserRole.Teacher);
...
{
    var userId = Guid.NewGuid();
    var superadmin = User.Create("sa", new byte[1], "SA", "L", Guid.NewGuid(), UserRole.Superadmin);
...
{
    var userId = Guid.NewGuid();
    var user = User.Create("u", new byte[1], "F", "L", Guid.NewGuid(), UserRole.Teacher);
...
public async Task GetAllUsersAsync_ShouldPassFilterToRepoAndReturnList()
{
    ...
    { 
         User.Create("u1", new byte[1], "A", "B", facultyId, UserRole.Student, "001").SetId(Guid.NewGuid()),
         User.Create("u2", new byte[1], "C", "D", facultyId, UserRole.Student, "002").SetId(Guid.NewGuid()),
         User.Create("u3", new byte[1], "E", "F", facultyId, UserRole.Student, "003").SetId(Guid.NewGuid())
    };
    var totalCount = 50;
