...
public class StudentExamGradeService : IStudentExamGradeService
{
    ...
    public async Task<StudentGradeListResponse> GetAllForExamAsync(int examId, Guid facultyId, int teacherId,
        CancellationToken ct)
    {
        ...
        return new StudentGradeListResponse
        {
            ...
            Grades = studentExamGrades.Select(g => new GradeResponse
            {
    ...
    public async Task<GradeResponse> CreateOrUpdateAsync(GradeRequest request, Guid facultyId, int teacherId,
        CancellationToken ct)
    {
        if (!await _repo.ExamBelongsToFacultyAndTeacherAsync(request.ExamId, facultyId, teacherId, ct))
        {
            _logger.LogWarning("Unauthorized POST attempt for ExamId={ExamId} StudentId={StudentId} by TeacherId={TeacherId}", 
                request.ExamId, request.StudentId, teacherId);
            throw new UnauthorizedAccessException("You can grade only your own exams.");
        }

        if (request.Points < 0 || request.Points > 100)
        {
            _logger.LogWarning("Invalid points: {Points} for StudentId={StudentId} ExamId={ExamId}", request.Points, request.StudentId, request.ExamId);
            throw new InvalidOperationException("Points must be between 0 and 100.");
        }

        var grade = await _repo.GetAsync(request.StudentId, request.ExamId, ct);
        ...
        if (grade == null)
        {
            ...
            _logger.LogInformation("Created grade for StudentId={StudentId} ExamId={ExamId} by TeacherId={TeacherId}", 
                request.StudentId, request.ExamId, teacherId);
        }
        else
        {
            grade.Points = request.Points;
            grade.Passed = request.Passed;
            grade.URL = request.Url;
    ...
    public async Task<GradeResponse> UpdateAsync(int studentId, int examId, GradeUpdateRequest request, Guid facultyId,
        int teacherId, CancellationToken ct)
    {
        ...
        if (request.Points.HasValue)
        {
            if (request.Points < 0 || request.Points > 100)
            {
                _logger.LogWarning("Invalid points update: {Points} for StudentId={StudentId} ExamId={ExamId}", request.Points, studentId, examId);
                throw new InvalidOperationException("Points must be between 0 and 100.");
            }
            grade.Points = request.Points.Value;
            grade.Passed = grade.Points >= 55;
        }

        if (request.Passed.HasValue) grade.Passed = request.Passed.Value;
        if (request.DateRecorded.HasValue) grade.DateRecorded = request.DateRecorded.Value;
